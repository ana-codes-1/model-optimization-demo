...
| Config-driven models | ✅ | The twenty-model roster, prompts, and price table all live in `config.json`; no model name appears in source |
...
| Rate limiting | ❌ | **Accepted risk, and the sharpest one here.** The endpoint is public and unauthenticated, and every run spends real tokens across twenty models. The budget alert below *notifies*, it does not cap. Real mitigation is operational: `make down` between demos. Add auth before leaving this running unattended |
